node {
    ...
    def secrets = [
        [
            ...
            secretValues: [
                [vaultKey: 'windows_pass'],
                [vaultKey: 'windows_user'],
                [vaultKey: 'linux_pass'],
                [vaultKey: 'linux_user'],
            ]
        ]
    ...
    def runAnsiblePlaybook(playbook, extraVars) {
        withCredentials([
            [$class: 'UsernamePasswordMultiBinding', credentialsId: "${NEXUS_CRED}", usernameVariable: 'NEXUS_USER', passwordVariable: 'NEXUS_PASSWORD']
        ]) {
            withVault([configuration: configuration, vaultSecrets: secrets]) {
                sh ""
                "
                ansible - playbook - i host--extra - vars "${extraVars}"
                $ {
                    playbook
                }
                ""
